...
app.get('/api/tasks', (req: Request, res: Response) => {
  ...
  if (
    ...
    PRIORITIES.includes(priorityParam as Priority)
  ) {
...
app.post('/api/tasks', (req: Request, res: Response) => {
  ...
  if (
    ...
    res
      .status(400)
      .json({
        error: `priority is required and must be one of: ${PRIORITIES.join(', ')}`,
      })
    return
  }
...
app.patch('/api/tasks/:id', (req: Request, res: Response) => {
  ...
  const priorityUpdate = body.priority
  if (priorityUpdate !== undefined) {
    if (
      typeof priorityUpdate !== 'string' ||
      !PRIORITIES.includes(priorityUpdate as Priority)
    ) {
      res
        .status(400)
