...
- **Filter and sort logic (App.tsx):** All filtering and sorting is done **in-memory** on the client. We fetch all tasks once on mount; filter/sort state drives a derived list (`filteredAndSortedTasks`) via `useMemo`, so the list updates instantly with no extra API calls.
  - **Filter by status:** Options are *All*, *Active*, *Completed*. Default *All*. Active = `!task.completed`; Completed = `task.completed`.
  - **Filter by priority:** Options are *All*, *High*, *Medium*, *Low* (order chosen for UX: most important first). Default *All*. When not All, we keep only tasks whose `priority` matches the selected value.
  - **Filter order:** Status filter is applied first, then priority filter, so the list is the intersection of both.
  - **Sort by:** User can choose *Priority (high first)*, *Newest first*, or *Oldest first*. Default is **Newest first**.
  - **Sort “Priority (high first)”:** (1) Active tasks first, completed tasks at the end. (2) Within each group: priority high → medium → low (`PRIORITY_RANK`: high=3, medium=2, low=1). (3) Tie-break by `createdAt` descending (newest first). So when viewing “All”, completed items appear after all active items.
  - **Sort “Newest first”:** Order by `createdAt` descending; tie-break by priority descending (high first).
  - **Sort “Oldest first”:** Order by `createdAt` ascending; tie-break by priority descending (high first).
  - **UI:** Minimal “Filter by status”, “Filter by priority”, and “Sort by” rows with small buttons; no separate “Filters” heading. Each control group has `role="group"` and an `aria-label`; sort/filter buttons use `aria-pressed` for the selected option.
